...
ELECTRON_FILES = ["https://duke.box.com/shared/static/vyiizlfvwz6tmfupey5hxluc45gkbvr3.root"]
...
def load_dropbox_filename(particle: str, index: int) -> list:
    ...
    if particle == "electron":
        assert index <= len(ELECTRON_FILES), f"Not enough electron files available for index: {index}"
        return ELECTRON_FILES[index]
    elif particle == "pion":
        assert index <= len(PION_FILES), f"Not enough pion files available for index: {index}"
